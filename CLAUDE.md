...
|---|---|---|
| Scaffolding + hash chain (M8 foundation) | done | 2026-05-15 |
| Witness Day 1: approvals in AuditDO, HMAC link tokens, go.kajaril.com worker + approve page | done | 2026-06-11 — see kajaril-witness-integration-draft.md §3; deployed 2026-07-12 |
| Witness Day 2: approval.requested/decided chain events, action-payload hashing (D6), signed decision webhook (D9), per-channel TTL, rate limiter | done | 2026-06-12 — webhook-secret design recorded as D9 in the draft; deployed 2026-07-12 |
| Witness Day 3: M2M client-credentials auth (hand-rolled, D2/D10), tool dispatch 4→6, REST approval surface, approval_url minting | done | 2026-06-12 — credential-storage design recorded as D10 in the draft; workers-oauth-provider bumped 0.7.2 → 0.8.1 (PR #30, then #38 — both founder-approved security-fix overrides of the 2-week quarantine); browser-flow wiring completed as part of Day 5; deployed 2026-07-12 |
| Witness Day 4: CF Access aud pin; channel ladder (D4) — Telegram cards + callbacks + deny-with-reason, one-time connect deep links, Resend email escalation via DO alarm, CHANNELS_KV routing config | done | 2026-06-12 — Telegram and Resend both confirmed live (bot about-text set, all 5 secrets bound including RESEND_API_KEY — no longer synthetic-only); browser-flow endpoint ownership resolved per D1 (go.kajaril.com owns every public human surface; the gated worker exposes none); deployed 2026-07-12 |
| Witness Day 5: D11 grant_type shim + browser OAuth (library-owned DCR/consent/code grants, all six conditions, adversarial tests); /verify page + human-readable dossier on go worker; dossier downloads off the gated worker (DossierInternal); DEPLOY.md runbook | done | 2026-06-13 — D8 magic-link still NOT built (founder decision still pending, see Day-5 report — unchanged); OAUTH_KV created and bound; deployed 2026-07-12 |
| Deploy day: all three Workers live (audit-event-notary, audit-event-mcp, go-kajaril custom domains all resolving), EU jurisdiction pin (locked decision 12), npm republished as v0.2.0 via trusted publishing (OIDC, no long-lived token) | done | 2026-07-12 |
