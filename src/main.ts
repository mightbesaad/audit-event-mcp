...
export class ApprovalInternal extends WorkerEntrypoint<Env> implements ApprovalInternalClient {
  private stubFor(clientId: string): DurableObjectStub {
    const doId = this.env.AUDIT_DO.idFromName(`audit-do-${clientId}`);
    return this.env.AUDIT_DO.get(doId);
  }
  ...
  async getApproval(clientId: string, approvalId: string): Promise<ApprovalRecord | null> {
    ...
    const res = await this.stubFor(clientId).fetch("https://do-internal/approval/get", {
      method: "POST",
  ...
  async decideApproval(params: {
    ...
    const res = await this.stubFor(params.clientId).fetch("https://do-internal/approval/decide", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
